...
private:
    ...
    std::vector<std::pair<std::string, int>> tf_map;
public:
    doc_t() = delete;
    ...
    doc_t(std::string path, std::string& text) {
        ...
        for (auto& t : tokens) {
            ...
            if(i == tf_map.size()) {
                tf_map.push_back(std::make_pair(t, 1));
            }
        }
    }
    ...
    std::vector<std::pair<std::string, int>> get_tf_map() const { return tf_map; }
    ...
    size_t get_bytes_count() {
        ...
        bytes += sizeof(tf_map);
        for(auto& p : tf_map) {
            bytes += sizeof(p.first) + p.first.size();
